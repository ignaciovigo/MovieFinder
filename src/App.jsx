...
function useSearch () {
  ...
  return { search, updateSearch, error, isFirstRender }
}

function App () {
  const { search, updateSearch, error, isFirstRender } = useSearch()
  ...
  return (
    <div className='page wave1'>
      <header>
        ...
        {error && <p style={{ color: 'red', margin: '0px',padding:'0px 0px 0px 4px' }}>{error}</p>}
      ...
      <main>
        {
        isFirstRender.current && !loading ? <h3>Busca cualquier pelicula pelicula!</h3> : loading ? <p>Cargando ...</p> : <ListMovies movies={movies}/>
        }
      </main>
    </div>
  )
